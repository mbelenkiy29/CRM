...
import { McaCommission, McaCommissionSplit, McaDeal, McaFunding, McaOffer, McaRenewal } from '../data/entities'
...
const createFundingCommand: CommandHandler<FundingCreateInput, CreateFundingResult> = {
    ...
    await withAtomicFlush(
      ...
      [
        () => {
          ...
          em.persist(em.create(McaRenewal, {
            organizationId: scope.organizationId,
            tenantId: scope.tenantId,
            fundingId,
            dealId: deal.id,
  ...
  undo: async ({ logEntry, ctx }) => {
    ...
    const offer = funding.offerId
      ...
      : null
    const renewals = await em.find(McaRenewal, { fundingId: funding.id, deletedAt: null })
    const now = new Date()
    ...
    await withAtomicFlush(
      ...
      [
        () => {
          ...
          for (const renewal of renewals) renewal.deletedAt = now
